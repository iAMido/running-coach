...
**Aerobic decoupling is GRADE-ADJUSTED — do not compare it to a TrainingPeaks number.** `runs.decoupling_pct` is computed in `lib/utils/decoupling.ts` from per-lap `gap_pace_min_km`, not raw speed, and `decoupling_method` records that (`lap_gap`). Raw Pa:HR is confounded by terrain: this athlete's quality sessions all climb out and descend back, so raw speed rises for free in the back half and *understates* decoupling; his steady runs lean the other way (Easy −14.6 s/km, Long Run −16.2 s/km of uphill adjustment). Gates: ≥6 laps carrying both pace and HR, halves within 40–60% of total time, and `Intervals`/`Fartlek` excluded entirely since work/recovery structure swamps the signal. No raw-speed fallback — a second, non-comparable measurement in the same column is how a metric quietly becomes meaningless.

**Friel's <5 / 5–8 / >8 bands are NOT applied as verdicts.** They are defined on raw Pa:HR and calibrated on other athletes. Measured on this athlete's own 68 runs: median 6.7%, and 27 runs exceed 8% — so applying them unchanged would label a third of his easy and long running "went too hard". That may be true (rebuilding at CTL 17.7 through an Israeli August, thermal drift raises Pa:HR exactly this way) or the threshold may not be his; there is not yet enough history to tell. The formatter therefore renders his own **percentile and median**, not a colour. Revisit once a season of data exists.
