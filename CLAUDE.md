...
**⚠️ Vercel Hobby allows exactly 2 cron jobs — `vercel.json` must never declare more.** It declared 3 for a long time (strava-sync ×2 + weekly-health-audit) and the surplus one was silently never registered: on 2026-08-06 the 15:00 UTC intervals-sync did not fire at all — no invocation in the runtime logs — while the 21:40 slot did. Adding a third cron does not error, it just quietly drops one, so the failure looks like a broken route rather than a plan limit. Two schedules on the *same* path still count as two jobs. Verify registrations in the Vercel dashboard under Settings → Cron Jobs, which lists what is actually scheduled rather than what was requested.

The surviving sync runs at 21:40 UTC = 00:40 Israel, deliberately after the evening window when runs actually happen (recent sessions start 18:00–20:30 local). A 15:00 UTC slot would be 18:00 local and would routinely miss the same day's run.
