...
The future of JavaScript tooling is fast, and Bun is leading the way.`,
    contentHtml: `
      <p class="lead">When building a modern web development boilerplate, every decision matters. The choice of package manager might seem small, but it impacts every developer who uses your template. Here's why we chose Bun.</p>

      <h2>Speed That Actually Matters</h2>
      <p>Bun installs packages up to 30x faster than npm. This isn't just a benchmark number — it's time saved on every <code>install</code>, every CI run, every new developer onboarding. When you run <code>bun install</code>, you'll notice the difference immediately.</p>
      <p>In our testing, a fresh install of this boilerplate's dependencies takes about <strong>2 seconds</strong> with Bun, compared to over <strong>30 seconds</strong> with npm. That's not a typo.</p>

      <h2>Native TypeScript Support</h2>
      <p>Bun runs TypeScript natively without any compilation step. This means faster development server startup, quicker test runs, and a simpler toolchain. No more waiting for tsc or dealing with complex build configurations just to run your code.</p>
      ...
      <p>Beyond raw performance, Bun focuses on developer experience. The CLI is intuitive, error messages are helpful, and the documentation is excellent. It feels like a tool built by developers who understand the daily frustrations of JavaScript development.</p>
...
For this boilerplate, shadcn/ui was the obvious choice. It gives you professional UI components while keeping full control in your hands.`,
    contentHtml: `
      <p class="lead">Component libraries have always been a trade-off. You get speed and consistency, but you lose control and flexibility. shadcn/ui changes that equation entirely.</p>

      <h2>The Copy-Paste Philosophy</h2>
      <p>Unlike traditional component libraries where you <code>npm install</code> and import, shadcn/ui gives you the actual source code. You copy it into your project, and it's yours. Want to modify a button? Just edit the file. No more fighting with CSS overrides or wrapping components in styled wrappers.</p>
      ...
      <p>Under the hood, shadcn/ui uses Radix UI primitives. This gives you world-class accessibility out of the box. Screen readers, keyboard navigation, focus management — it all just works.</p>

      <h2>Tailwind CSS Integration</h2>
      <p>Every component is styled with Tailwind CSS, making customization intuitive. If you know Tailwind, you already know how to customize these components. The utility classes are right there in the code, ready to be tweaked.</p>

      <h2>The Right Level of Abstraction</h2>
      <p>shadcn/ui hits the sweet spot between too primitive and too opinionated. The components are complete enough to use immediately but simple enough to understand and modify. You're not fighting a black box.</p>
...
With Next.js 14, the App Router is stable and performant. The rough edges have been smoothed, the bugs have been fixed, and the documentation is comprehensive. It's time to build.`,
    contentHtml: `
      <p class="lead">Next.js 14 represents the maturation of the App Router. After months of improvements, it's now ready for production use. Here's what you need to know.</p>

      <h2>Server Components by Default</h2>
      <p>In the App Router, components are Server Components by default. This means they render on the server, reducing the JavaScript sent to the client. Your pages load faster, and your users are happier.</p>
      ...
      <p>Gone are <code>getServerSideProps</code> and <code>getStaticProps</code>. Now you just fetch data directly in your components using async/await. The mental model is simpler, and the code is cleaner.</p>

      <h2>Layouts and Templates</h2>
      <p>The App Router introduces a powerful layouts system. Define a layout once, and it wraps all child routes. Combined with loading states and error boundaries, you can build sophisticated UIs with minimal boilerplate.</p>

      <h2>Streaming and Suspense</h2>
      <p>Next.js 14 fully embraces React Suspense. Pages stream to the browser piece by piece, showing content as it becomes available. No more waiting for the entire page to load.</p>

      <h2>Production Ready</h2>
      <p>With Next.js 14, the App Router is stable and performant. The rough edges have been smoothed, the bugs have been fixed, and the documentation is comprehensive. It's time to build.</p>
    `,
  },
];
